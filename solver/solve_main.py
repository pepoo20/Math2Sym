...
def remove_commas_in_numbers(input_string):
    # Define a regex pattern to find numbers with commas
    pattern = r'\b\d{1,3}(?:,\d{3})+\b'
    
    # Function to replace commas in numbers
    def replace_commas(match):
        return match.group().replace(',', '')
    
    # Apply the regex pattern and replace commas in numbers
    result = re.sub(pattern, replace_commas, input_string)
    return result

def parse_and_simplify_equation(equation):
    parse_equation = equation
    parse_equation = remove_commas_in_numbers(parse_equation)
